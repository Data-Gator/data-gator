...
SensorFactory sensorFactory(config_filename);
...
void init_nvs()
{
	...
	if (gator_prefs.isKey("reset_count"))
	{ // if key exists
...
void ReadWired()
{
	...
	if (DEBUG)
		Serial.println("[VWC/WIRED SENSORS] queueing data");
	...
	{	
		if (sensors[i] != nullptr)
		{
			...
			std::string topic = brand + "/" + std::to_string(i) + "_" + depth + "/" + mac_str;
			// std::string msg = "{\"MAC\": \"" + mac_str + "\", " + sensors[i]->toJSON(voltage[i]) + "}";
			std::string msg = "{\"MAC\": \"" + mac_str + "\", \"DEPTH\": \"" + depth + "\", " + sensors[i]->toJSON(voltage[i]) + "}";
			log_data(topic, msg);
		}
		else
		{
...
void Scheduler(int reset_count)
{
	...
	{
		if (DEBUG)
			Serial.println("[ERROR] over ran max reset count without reseting count, check if variable is being reset or if tasks are not completing");
